...
class TemplateBlock(nn.Module):
    """Block used for template features only, with self-attention only."""
    ...
    def forward(self, x):
        """x: (B, N, C) containing only template tokens."""
        B, N, C = x.shape
...
if __name__ == "__main__":
    print("To export correctly, update lib/models/mixformer2_vit/head.py. "
            "The original model keeps this tensor on CUDA, but for the split export "
            "it should be moved to CPU:\n"
