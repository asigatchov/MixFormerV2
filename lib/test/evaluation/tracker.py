...
class Tracker:
    ...
    def run_video(self,
        ...
        # Initialize the video writer if requested.
        if save_video:
        ...
        # Initialize the ROI output directory.
        roi_base_dir = None
        roi_class_dir = None
        if save_roi:
            if roi_output_dir is None:
                # By default, create an output folder next to the video file.
            ...
            # If a class was detected, create a class-specific subdirectory.
        ...
        while True:
            ...
            if expansion_ratio != 1.0:
                x, y, w, h = state
                # Compute the expanded width and height.
                new_w = int(w * expansion_ratio)
                new_h = int(h * expansion_ratio)
                # Adjust x and y so that the center point stays fixed.
                x = x - (new_w - w) // 2
                y = y - (new_h - h) // 2
            ...
            if save_yolo:
                ...
                # Use the video filename as part of the save directory name.
                video_name = Path(videofilepath).stem
                save_dir = os.path.join(
            ...
            # Save ROI crop images.
            if save_roi and roi_class_dir is not None:
                # Get the target center point and size.
                x, y, w, h = state
                center_x = x + w // 2
                center_y = y + h // 2
                ...
                if max_side > roi_size:
                    # If the target is larger than the requested crop, use the target size.
                    crop_size = max_side
                else:
                    # Otherwise use the requested ROI size.
                    crop_size = roi_size
                ...
                # Read the original frame size.
                frame_h, frame_w = frame.shape[:2]
                
                # Compute the padding required around the crop.
                pad_left = max(0, -x1)
                pad_top = max(0, -y1)
                ...
                # Crop the valid frame region.
                roi_crop = frame[y1_clip:y2_clip, x1_clip:x2_clip].copy()
                
                # Pad the crop if it extends beyond the frame.
            ...
            # Write the output video frame if requested.
            if save_video:
